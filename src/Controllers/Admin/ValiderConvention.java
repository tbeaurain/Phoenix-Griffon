...
/**
 * Servlet implementation class ValiderConvention
 */
@WebServlet(name="/ValiderConvention", urlPatterns={"/validerConvention"})
public class ValiderConvention extends HttpServlet {
...
public static final String VUE_SUCCES = "/WEB-INF/Admin/AfficherConvention.jsp";
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(user.getStatutUtilisateur().getId()==2){
			Stage stageEffectue = stagesEffectuesListe.get(0).getId_stage();
			ValideStageDAO vsdao = new ValideStageDAO();
			ValideStage vs = new ValideStage(eleve,stageEffectue);
			vsdao.create(vs);
		}
		
		eleve = usrbdd.find(eleveID);
		request.setAttribute(ATT_ELEVE, eleve);
		
		
		stagesEffectuesListe = new ArrayList<>(eleve.getEffectueStages());
		stagesValidesListe = new ArrayList<>(eleve.getValideStages());
